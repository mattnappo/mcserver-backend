...
func Execute(command, name string) (string, error) {
	exec.Command("/bin/sh", "systemctl "+command+" "+name)      // Execute the systemctl command
	status := exec.Command("/bin/sh", "systemctl status "+name) // Get the status of the service
